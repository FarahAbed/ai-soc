...
Parse records. Read all events. Note total record count, the
EventID distribution (how many EID 1, 3, 11), and any records with
missing required fields.
Build process trees. Using EventID 1 records, reconstruct
...
Flag suspicious chains. For each chain, evaluate whether the
...
Flag LOLBin invocations. Scan all EventID 1 `CommandLine` and
...
Flag persistence writes. Scan all EventID 11 records. For each
`TargetFilename` matching a persistence path pattern, record the
writing process (`Image`, `ProcessId`) and the target path.
Evaluate NetworkConnect context. For EventID 3 records, check
...
Assess severity. Use the following rubric:
...
Assign confidence. Set `confidence` between 0.0 and 1.0:
...
Map ATT&CK. Assign at least one ATT&CK technique ID from the
in-scope list to the primary finding. Include sub-technique IDs where
the evidence supports them.
Formulate recommendation. State the next analyst action. If
...
Perform self-check. Before returning output, silently verify:
All 8 shared keys are present.
`suspicious_chains` and `lolbins` are present and are arrays.
...
```json
{
  ...
  "suspicious_chains": [
    {
      "parent_image": "<full path of parent process>",
      "parent_pid": <integer or null>,
      "child_image": "<full path of child process>",
      "child_pid": <integer or null>,
      "command_line": "<command line of child, or null if absent>",
      "rule_fired": "<which parent-child rule or LOLBin rule triggered this entry>"
    }
  ],
  "lolbins": [
    {
      ...
      "pid": <integer or null>,
      "command_line": "<full command line, or null if absent>",
      "parent_image": "<spawning process, or null if unresolved>",
...
`severity: "info"`, empty arrays for `suspicious_chains` and `lolbins`,
...
No hallucination. Every entry in `evidence`, `suspicious_chains`,
